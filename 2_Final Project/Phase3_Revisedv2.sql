...
INSERT INTO cs4400spring2020.`User`(username,`password`,firstname,lastname) VALUES("Staff10", "4242424242", "Staff10", "Staff10"), ("Staff11", "4242424242", "Staff11", "Staff11");
...
INSERT INTO cs4400spring2020.Employee(username,email) VALUES ("Staff11","school11@gmail.com"), ("Staff10","school10@gmail.com");
...
-- Added to show functionality for staff members not assigned to a food truck.
INSERT INTO cs4400spring2020.Staff(username,foodTruckName) values ("Staff10", null), ("Staff11", null);
...
BEGIN
	IF length(i_password) < 8 THEN
		LEAVE register_main;
	ELSEIF EXISTS (SELECT * FROM `User` WHERE username = i_username) THEN
		LEAVE register_main;
    ELSEIF EXISTS (SELECT * FROM `User` WHERE firstname = i_firstname AND lastname = i_lastname) THEN
		LEAVE register_main;
	ELSEIF i_balance < 0 THEN
		LEAVE register_main;
	ELSEIF i_email IS NULL AND i_balance IS NULL THEN
		LEAVE register_main;
	ELSEIF i_email IS NOT NULL AND EXISTS (SELECT * FROM employee WHERE email = i_email) THEN
		LEAVE register_main;
	END IF;
...
BEGIN
    ...
    DECLARE usedcapacity INT default 0;
    DECLARE totalcapacity INT default 0;
	DROP TABLE IF EXISTS mn_filter_foodTruck_remainingcapacity;
	CREATE TABLE mn_filter_foodTruck_remainingcapacity (uniqueID INT NOT NULL AUTO_INCREMENT, stationName varchar(100), buildingName varchar(100), capacity INT not null, 
    PRIMARY KEY (uniqueID));
    insert into mn_filter_foodTruck_remainingcapacity (stationName, buildingName, capacity) (SELECT * FROM station);
	SELECT COUNT(*) FROM STATION INTO n;
    set i = 1;
    WHILE i <= n DO
		SET usedcapacity = (SELECT COUNT(*) FROM foodtruck WHERE stationName = (SELECT stationName from mn_filter_foodTruck_remainingcapacity where uniqueID = i));
        SET totalcapacity = (SELECT capacity FROM mn_filter_foodTruck_remainingcapacity WHERE uniqueID = i);
		UPDATE mn_filter_foodTruck_remainingcapacity SET capacity = totalcapacity - usedcapacity WHERE uniqueID = i;
        SET i = i + 1;
	END WHILE;
	...
	SELECT foodtruck.foodtruckname, foodtruck.stationName, max(mn_filter_foodTruck_remainingcapacity.capacity), COUNT(DISTINCT username) as "Staff(s)", COUNT(DISTINCT foodName) as'#Menu Item'
    FROM FOODTRUCK
    INNER JOIN mn_filter_foodTruck_remainingcapacity ON mn_filter_foodTruck_remainingcapacity.stationName = foodtruck.stationName
	INNER JOIN STAFF ON staff.foodTruckName = foodtruck.foodtruckname
    INNER JOIN MENUITEM ON menuitem.foodTruckName = foodtruck.foodtruckname
    INNER JOIN (SELECT COUNT(*) as num_staff, foodtruckname FROM staff GROUP BY foodtruckname) as staff_count ON foodtruck.foodtruckname = staff_count.foodtruckname
    WHERE
    (i_managerUsername = managerUsername) AND
    (foodtruck.foodtruckname LIKE CONCAT('%', i_foodTruckName, '%') OR i_foodTruckName IS NULL OR i_foodTruckName = "") 
	AND (foodtruck.stationName = i_stationName OR i_stationName IS NULL OR i_stationName = "")  
    AND ((i_hasRemainingCapacity = TRUE AND capacity > 0) OR (i_hasRemainingCapacity = FALSE))
    AND ((i_minStaffCount IS NULL OR i_minStaffCount = "" OR i_minStaffCount <= staff_count.num_staff) AND (i_maxStaffCount IS NULL OR i_maxStaffCount = "" OR staff_count.num_staff <= i_maxStaffCount))
...
create_foodtruck:
BEGIN
	IF NOT EXISTS(select * from station where Stationname = i_stationName) THEN 
		LEAVE create_foodtruck;
	ELSEIF NOT EXISTS (select * from Station join foodTruck using (stationName) WHERE (station.capacity > 0) AND stationName = i_stationName) THEN
    LEAVE create_foodtruck;
	END IF;

	IF NOT EXISTS (select * from foodtruck where foodtruckname = i_foodtruckname) THEN
...
BEGIN
	IF i_price > 0 AND EXISTS (SELECT * FROM foodtruck WHERE foodtruckname = i_foodtruckname) AND EXISTS (SELECT * FROM food WHERE foodname = i_foodname) 
	AND NOT EXISTS (SELECT * FROM menuitem WHERE foodname = i_foodname AND foodtruckname = i_foodtruckname) THEN
...
BEGIN
	DROP TABLE IF EXISTS mn_view_foodTruck_available_staff_result;
     ...
     from user join staff using (username) 
...
BEGIN
    ...
    INNER JOIN USER
    using(username) 
    where staff.foodTruckName = i_foodtruckname;
...
BEGIN
	...
	insert into mn_view_foodTruck_menu_result select foodtruck.foodTruckName, foodtruck.stationName, menuitem.foodName, menuitem.price 
...
BEGIN
	IF EXISTS (SELECT * FROM foodtruck WHERE foodtruckname = i_foodtruckname) AND EXISTS (SELECT * FROM staff where username = i_staffName) THEN
		UPDATE STAFF
...
BEGIN
	IF EXISTS (SELECT * FROM foodtruck WHERE foodtruckname = i_foodtruckname) AND EXISTS (SELECT * FROM food WHERE foodname = i_foodname) THEN
		UPDATE MenuItem
		SET price = i_price
...
BEGIN
	...
	IF EXISTS (SELECT * FROM manager WHERE username = i_managerUsername) THEN
...
BEGIN
	...
	IF EXISTS (SELECT * FROM manager WHERE username = i_managerUsername) AND EXISTS (SELECT * FROM foodtruck WHERE foodtruckname = i_foodtruckname) THEN
		INSERT INTO mn_summary_detail_result
		SELECT Orders.date AS `date`, CONCAT(firstName , ' ' , lastName) AS "Customer", SUM(price * purchaseQuantity) AS "Total Purchase", COUNT(DISTINCT (Orders.orderID)) AS "# Orders",  GROUP_CONCAT(DISTINCT(OrderDetail.foodName) SEPARATOR ', ') AS "Food(s)"
...
BEGIN
    IF i_stationName in (SELECT stationName FROM Station) AND i_customerUsername in (SELECT username FROM customer)
    THEN
...
BEGIN
	...
	balance DECIMAL(6, 2));
	IF EXISTS (SELECT * FROM customer WHERE username = i_customerUsername) THEN
    
		INSERT INTO cus_current_information_basic_result SELECT stationName, station.buildingName, GROUP_CONCAT(DISTINCT tag SEPARATOR ', ') 
		AS tags, description, balance 
		FROM building NATURAL JOIN station NATURAL JOIN buildingtag NATURAL JOIN customer
		WHERE username = i_customerUsername;
...
INSERT INTO cus_current_information_foodTruck_result SELECT foodtruck.foodTruckName, CONCAT(`user`.firstName, " ", `user`.lastName), GROUP_CONCAT(DISTINCT menuitem.foodName SEPARATOR ', ') 
FROM menuitem join foodtruck on menuitem.foodtruckname = foodtruck.foodtruckname
join Customer on customer.stationName = foodtruck.stationName
join manager on manager.username = foodtruck.managerUsername
join `user` on manager.username = `user`.username

where customer.username = i_customerUsername GROUP BY foodtruck.foodtruckname;
...
cus_add_to_order:
    BEGIN
    IF EXISTS (SELECT * FROM orderdetail WHERE foodname = i_foodname AND foodtruckname = i_foodtruckname AND orderID = i_orderID) THEN
		leave cus_add_to_order;
	END IF;
	insert into orderdetail VALUES(i_orderId, i_foodTruckName, i_foodname, i_purchaseQuantity);
	SET @old_balance2 = (select balance from Orders join Customer on customer.username = orders.customerusername where orderID = i_orderId);
	SET @order_total = i_purchasequantity*(select price from menuitem where foodName = i_foodName and foodtruckname = i_foodTruckName);
	update customer SET customer.balance =  @old_balance2 - @order_total where username = (select customerUsername from Orders where orderID = i_orderId);
    
	IF i_purchaseQuantity < 0 OR i_purchasequantity*(select price from MenuItem where foodName = i_foodName and foodtruckname = i_foodTruckName) > @old_balance2 THEN
    
	SET @old_balance = (select balance from Orders join Customer on customer.username = orders.customerusername where orderID = i_orderId);
	SET @order_cost = (select sum(purchasequantity*price) from orderdetail join menuitem using (foodname,foodtruckname) where orderID = i_orderId);
	update customer SET balance = @old_balance + @order_cost where username = (select customerUsername from Orders where orderID = i_orderId);
	DELETE from orderdetail where orderID = i_orderId;
	DELETE from orders where orderID = i_orderId;
...
BEGIN
	...
	IF EXISTS (SELECT * FROM customer WHERE username = i_customerUsername) THEN
		insert into cus_order_history_result select date, orderID, sum(purchaseQuantity * price) as OrderTotal, GROUP_CONCAT(DISTINCT foodName SEPARATOR ', ') as 'Food(s)', sum(purchasequantity) as 'Food Quantity'
		from orderdetail join menuitem using (foodtruckname, foodname) right join orders using(orderID)
...
-- Query: used for GUI Screens 12 amd 13 
DROP PROCEDURE IF EXISTS mn_filter_foodTruck_hascapacity;
DELIMITER //
CREATE PROCEDURE mn_filter_foodTruck_hascapacity()
BEGIN
    DECLARE n INT default 0;
    DECLARE i INT default 0;
    DECLARE usedcapacity INT default 0;
